...
float speed = 400; //variable float 'speed'
void setup() {
  size(960, 540);
  stroke(255);
  background(255);
...
void draw() { 
  ...
  if(aend > width) {
    aend = aend*speed + 540 ;
    astart = astart*speed + 540;
  }
  if (random(400)>300) { // to randomize the bar colour
  stroke(0); 
  } else {
  ...
  saveFrame("screen-3879.png"); //save each frame as screen
}